...
function showLoading(isLoading) {
    ...
    if (searchButton) {
        if (isLoading) {
            searchButton.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Mencari...';
...
function showSearchDistribution() {
    ...
    // Tampilkan modal
    const distributionModal = new bootstrap.Modal(document.getElementById('searchDistributionModal'));
    distributionModal.show();
    
    // Tampilkan loading state
    document.getElementById('distributionChartContainer').innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"></div><p class="mt-2">Memuat distribusi...</p></div>';
